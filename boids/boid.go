...
func (b *Boid) calcAccelaration() Vector2D {
	...
	for i := math.Max(lower.x, 0); i <= math.Min(upper.x, screenWidh); i++ {
...
func (b *Boid) moveOne() {
	b.velocity = b.velocity.Add(b.calcAccelaration()).Limit(-1, 1)
	boidMap[int(b.position.x)][int(b.position.y)] = -1
	b.position = b.position.Add(b.velocity)
	boidMap[int(b.position.x)][int(b.position.y)] = b.id
	next := b.position.Add(b.velocity)
	if next.x > screenWidh || next.x < 0 {
...
func createBoid(bid int) {
	b := Boid{
		position: Vector2D{rand.Float64() * screenWidh, rand.Float64() * screenHeight},
